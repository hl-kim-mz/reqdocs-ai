...
import { Copy, Download, RefreshCw, Pencil, Check, X } from 'lucide-react';
...
export default function SidePanel({
  ...
  return (
    <div className="w-72 bg-white rounded-lg border border-slate-200 p-6 flex flex-col gap-6">
      {/* Document Info */}
      <div>
        <p className="text-xs font-semibold text-slate-500 uppercase mb-2">문서 정보</p>
        <div className="space-y-2">
          <div>
            <p className="text-xs text-slate-500">문서 타입</p>
            <p className="text-sm font-semibold text-slate-900">{docTypeLabels[docType]}</p>
          </div>
          <div>
          ...
          <>
            {onEditStart && (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 disabled:opacity-40 transition-colors font-medium text-sm"
              >
                <Pencil size={16} />
                편집
              </button>
      ...
      {!isEditing && (
        <div className="border-t border-slate-200 pt-6">
          <p className="text-xs font-semibold text-slate-500 uppercase mb-2">통계</p>
